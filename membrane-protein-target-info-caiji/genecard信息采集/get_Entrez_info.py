...
def get_genecards_info(gene_symbol, cookies):
    ...
    if response.status_code == 200:
        ...
        # 获取 Entrez Gene Summary 信息
        summary_xpath = '/html/body/div[1]/div[2]/div/div/main/div[2]/div/div/section[2]/div[1]/ul/li'
        summary_element = tree.xpath(summary_xpath)

        if summary_element:
            summary = summary_element[0].text_content().strip().replace('\n', ' ').replace('\r', ' ')
        else:
            summary = '未找到 Entrez Gene 摘要信息'
